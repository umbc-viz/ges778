...
A better way to smooth out data like this is rolling averages. I usually use the `tsibble` and `slider` packages for making time series data frames and sliding averages, respectively. (You can do this without a special time series data type but I'm always nervous I'll mess up a time series, so I opt for tools specially designed for it.)
...
```{r}
md_unemp_rolling |>
    dplyr::filter(!is.na(rate_3mo)) |>
    tibble::as_tibble() |>
    dplyr::rename(rate_measured = rate) |>
    tidyr::pivot_longer(
        dplyr::matches("rate_"),
    ...
    dplyr::mutate(date = as.Date(date)) |>
    ggplot(aes(x = date, y = rate, linetype = type, color = type)) +
    geom_line() +
    scale_color_manual(values = c(measured = "gray50", "3mo" = "gray10")) +
    scale_linetype_manual(values = c(measured = "21", "3mo" = "solid")) +
    theme_minimal()
```
...
![From @smith2023](https://ygo-assets-websites-editorial-emea.yougov.net/images/Word20sentiment20US20variant-01.format-webp.webp){width=50%}
